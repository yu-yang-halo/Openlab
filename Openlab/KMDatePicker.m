...
#define kRowDisabledStatusColor [UIColor blackColor]
#define kRowNormalStatusColor [UIColor blueColor]
#define kWidthOfTotal self.frame.size.width
#define kHeightOfButtonContentView 35.0
#define kButtonNormalStatusColor [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.8]
...
- (instancetype)initWithFrame:(CGRect)frame block:(DateSelectBlock)block datePickerStyle:(KMDatePickerStyle)datePickerStyle{
    _block = block;
    _datePickerStyle = datePickerStyle;
    return [self initWithFrame:frame];
}
...
- (void)loadData {
    ...
    for (NSInteger i=0; i<kMinuteCountOfEveryHour; i++) {
        ...
        [_mArrMinute addObject:[NSString stringWithFormat:@"%02ld", (long)i*30]];
...
- (void)drawRect:(CGRect)rect {
    ...
    btnScrollToNow.frame = CGRectMake(buttonContentView.frame.size.width/2 - 70.0, 2.5, 140.0, kHeightOfButtonContentView - 5.0);
    btnScrollToNow.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    [btnScrollToNow setTitle:_title forState:UIControlStateNormal];
    [btnScrollToNow setTitleColor:kButtonNormalStatusColor forState:UIControlStateNormal];
    [btnScrollToNow setEnabled:YES];
...
- (void)playDelegateAfterSelectedRow {
    ...
    if(_block!=nil){
        _block(self,_datePickerDateScrollTo);
    }
}
...
-(void)show{
    myKeyWindow=[[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    [myKeyWindow setWindowLevel:UIWindowLevelAlert];
    [myKeyWindow addSubview:self];
    [myKeyWindow makeKeyAndVisible];
    
}
-(void)hide{
    [self removeFromSuperview];
    [myKeyWindow setHidden:YES];
    myKeyWindow=nil;
